                ...
                if res == true {
                    Ok(
                        web::block(move || add_space_db(db, item, auth.token().to_string()))
                            ...
                            .map_err(|_| HttpResponse::Ok().json(Response::new(false, "some error ocuured".to_string())))?,
                    )
                } else {
    ...
    match auth::validate_token(&token.token().to_string()) {
        Ok(res) => {
            if res == true {
                ...
                let space=spaces.filter(spaces_name.ilike(&space_name.info))
                .first::<Space>(&conn);

                match space{
                    Ok(space)=>{
                        let spaces_user:SpaceUser=spaces_users.filter(spaces_id.eq(space.id))
                .filter(user_id.eq(user.id))
                .first::<SpaceUser>(&conn).unwrap();
                ...
                while let Ok(Some(mut field)) = payload.try_next().await {
                    ...
                    if metadata.len() > 5000000 {
                        ...
                        match uploaded {
                            Ok(file_link) => {
                                let _space_details = diesel::update(
